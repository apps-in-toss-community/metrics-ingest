...
Cloudflare Workers + D1 기반 서비스로, 커뮤니티가 만든 개발 도구(`@ait-co/devtools`, `@ait-co/console-cli`)에서 발생하는 소규모 구조화 이벤트를 수집합니다. 존재 목적은 단 하나: **사람들이 우리 도구를 얼마나 활발히 쓰고 있는지**를 파악해, 언제 `1.0.0` 안정성 약속을 할 수 있을지 판단하는 것입니다.
...
클라이언트(`@ait-co/devtools`, `@ait-co/console-cli`)는 **opt-in** 방식입니다: 사용자가 명시적으로 동의하기 전까지 텔레메트리는 꺼져 있습니다. 동의 요청은 최대 두 번(초기 + 30일 후 재요청)으로 제한되며, 이후에는 다시 묻지 않습니다. 사용자는 언제든지 devtools 패널에서 텔레메트리를 켜거나 끌 수 있습니다.
